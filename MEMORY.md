...
- **id:** T1-2b-iv
- **title:** `fw-match-sim` signature dispatcher + first 3 signatures end-to-end (FINAL T1-2b row)
- **started:** 2026-05-15
- **task class:** sim-rust (canonical-state extension + new sub-module + utility-scoring integration; ≥100 LoC; gameplay-programmer)
- **required subagent:** `gameplay-programmer`
- **TDD mandate:** **YES** — seventh row under the superpowers TDD mandate. RED-GREEN-REFACTOR per chunk. **Iii-c/iii-d lesson applies prospectively: no test deferrals. The acceptance criteria (3 signatures fire + cooldown enforced + softmax deterministic + bias multiplies into utility + MemoryEvent emitted) ARE the test contract.**
- **Canonical-hash rebaseline:** AUTHORIZED — `signature_cooldowns: BTreeMap<(PlayerId, SignatureId), Tick>` joins canonical MatchState per ADR-0011 §"Cooldown" + ADR-0012 trigger #1 (canonical schema bump). Both `PINNED_60_TICK` + RON fixture `expected_hash` update atomically.

### Design references
- `docs/adr/0011-signature-system.md` — THE source-of-truth. §"Dispatch + softmax" specifies the eval-triggers + sample-via-softmax + set-signature_firing flow. §"Cooldown" specifies the canonical-state field + 600-tick default. §"Stacking policy" specifies same-category exclusion via `StackingPolicy::Exclusive { category }`. §"Counterplay" specifies defensive-signature cancellation predicates.
- `docs/adr/0005-memory-event-schema.md` (or whatever the ledger ADR is) for `MemoryEvent::SignatureFirstFired` emission shape.
- `docs/adr/0009-rng-seed-derivation.md` — `SeedLayer::SignatureTrigger` for softmax tie-breaking; site = `(player_id << 16) | local_decision_counter` per the cadence pattern.
- `crates/fw-content/src/signature.rs` (T1-3) — types frozen. `SignatureDefinition`, `SignatureId`, `SignatureCandidate`, `SimBiasSnapshot`, `CooldownPolicy`, `StackingPolicy`, `SignatureTrigger`. T1-2b-iv consumes these.
- `crates/fw-match-sim/src/bt/` (T1-2b-iii-c) — utility-scoring entry points. The bias snapshot multiplies INTO `apply_shoot_bias`/`apply_long_pass_bias`/etc. as a second multiplicative factor.
- v1: `MatchSim/Sim/SignatureRules.cs` + `SignatureCooldownState.cs` + `SignaturePresentationRecipe.cs` — design intent only; Rust idioms.

### Acceptance criteria (verbatim from MASTER_PLAN T1-2b-iv)
1. **3 signatures fire in test fixtures** — one defensive (`fwh.core:signature.body-shield-pressure`), one attacking (`fwh.core:signature.long-range-strike`), one build-up (`fwh.core:signature.first-time-diagonal-switch`). Each has its own RON fixture under `content/sources/signatures/` + a Rust trigger function bound to its `SignatureId` in fw-match-sim's signature module. Each fires in a constructed test fixture where the trigger predicate is satisfied.
2. **Cooldown enforced** — per-player+per-signature cooldown state in `MatchState.signature_cooldowns: BTreeMap<(PlayerId, SignatureId), Tick>` (BTreeMap-keyed for determinism). Default 600-tick cooldown per `CooldownPolicy::EveryTicks(600)`. Test: signature that fires at tick 100 with cooldown 600 cannot re-fire until tick 700.
3. **Softmax dispatch deterministic via `SeedLayer::SignatureTrigger`** — when multiple candidates fire simultaneously for the same player, softmax over `affinity × event-class-fit` with `seed_fn(match_seed, tick, SeedLayer::SignatureTrigger, (player_id << 16) | local_decision_counter)` picks one deterministically. Test: same input → same picked signature across reruns.
4. **Bias snapshot multiplies into utility scoring** — while a signature is in flight (`signature_firing.is_some()` for the player), the per-tick BT utility scoring multiplies the relevant per-consideration factor by the signature's `SimBiasSnapshot` field. E.g. `LongRangeStrike` firing on a player multiplies `shoot_mul = 1.4` into that player's shoot utility for the duration window. Test: utility for a player with-vs-without firing signature differs by the documented factor.
5. **`MemoryEvent::SignatureFirstFired` emitted** — when a signature fires for the FIRST time in a match for a given player+signature pair, emit `MemoryEvent::SignatureFirstFired { player_id, signature_id, tick }` to the ledger. T1-4 owns the ledger output struct + commentary; this row emits the event into the canonical event stream. If `MemoryEvent` doesn't exist yet (T1-4), define a LOCAL stub enum in fw-match-sim/src/signature/ledger.rs mirroring tactic_fsm's TacticEvent pattern; T1-4 reconciles.
6. **Canonical hash REBASELINED** intentionally. `signature_cooldowns` field joins `MatchState`; encoder VERSION 4 → 5 (schema bump); old `1db6020c…59c798` → new BLAKE3 TBD.

### Files in scope
- `crates/fw-match-sim/src/signature/mod.rs` (NEW; module-level coordinator)
- `crates/fw-match-sim/src/signature/triggers.rs` (NEW; 3 trigger predicate functions: `body_shield_pressure_trigger`, `long_range_strike_trigger`, `first_time_diagonal_switch_trigger`; each takes `(&MatchState, PlayerSlot)` and returns `bool` per ADR-0011 §"Trigger")
- `crates/fw-match-sim/src/signature/dispatcher.rs` (NEW; `evaluate_signatures(state, player_slot, signature_definitions) -> Option<SignatureId>`; softmax-samples via seed_fn per acceptance criterion #3)
- `crates/fw-match-sim/src/signature/bias_apply.rs` (NEW; `apply_signature_bias(raw_utility, signature_definition, consideration: BiasConsideration) -> Q32` — composes the `SimBiasSnapshot` multiplier into utility scoring per ADR-0011 §"Bias snapshot")
- `crates/fw-match-sim/src/signature/ledger.rs` (NEW; local stub `MemoryEvent::SignatureFirstFired` mirroring TacticEvent pattern; T1-4 reconciles)
- `crates/fw-match-sim/src/lib.rs` (MODIFIED; mod declaration + MatchState gains `signature_cooldowns: BTreeMap<(PlayerSlot, SignatureId), Tick>` + `signature_firing: [Option<(SignatureId, Tick)>; 22]` per-player + `signature_first_fired_seen: BTreeSet<(PlayerSlot, SignatureId)>` for the SignatureFirstFired one-shot; encoder VERSION 4 → 5)
- `crates/fw-match-sim/src/canonical.rs` (MODIFIED; encoder extension for new MatchState fields; wire-format diagram updated)
- `crates/fw-match-sim/src/dispatch.rs` (MODIFIED; per-tick: evaluate signature triggers for each player → softmax pick → set signature_firing + cooldown + emit SignatureFirstFired event)
- `crates/fw-match-sim/src/bt/on_ball.rs` + `off_ball.rs` (MODIFIED; bias_apply composition: when player has `signature_firing.is_some()`, the relevant utility scorer multiplies in the SimBiasSnapshot factor)
- `content/sources/signatures/body-shield-pressure.ron` (NEW; defensive — `StackingPolicy::Exclusive { category: Defensive }`)
- `content/sources/signatures/long-range-strike.ron` (NEW; attacking — `Exclusive { Attacking }`; `SimBiasSnapshot { shoot_mul: 1.4, dribble_mul: 0.7, ... }`)
- `content/sources/signatures/first-time-diagonal-switch.ron` (NEW; build-up — `Exclusive { BuildUp }`)
- `content/sources/players/*.ron` (MODIFIED; sample-am.ron expands signature_candidates to reference all 3 real signatures with documented affinities; other player fixtures optionally get candidates)
- `crates/fw-match-sim/tests/signature_dispatcher_proptest.rs` (NEW; 6+ proptest invariants covering acceptance criteria 1-5)
- `crates/fw-match-sim/tests/signature_triggers_unit.rs` (NEW; per-trigger unit tests)
...
- `crates/fw-content/**` — schema FROZEN at T1-3. iii-c-style validator drift = scope leak.
- `crates/fw-core/**` — Q32 + math LUTs + PlayerAttributes + SeedLayer locked.
- `crates/fw-tauri/**` / `frontend/**` — no IPC or UI changes.
- `crates/fw-match-sim/src/{ball*,ball_physics,tactic_fsm,decision_cadence,player,role_states,goalkeeper_fsm,subtree_library,utility,separation}.rs` — prior rows FROZEN. Only `lib.rs`, `canonical.rs`, `dispatch.rs`, `bt/on_ball.rs`, `bt/off_ball.rs` get the documented modifications.
- `scripts/determinism-audit.py` — no changes needed.

### Intentionally NOT done in this task
- Full 24-signature catalogue — only 3 representatives. T2-4 fills the catalogue.
- Counterplay cancellation predicates (`BodyShieldPressure` cancels nearby `LowCutbackByline`) — full cancellation defers to T2; iii-d ships only the 3 trigger predicates without cross-signature cancellation logic. `body_shield_pressure_trigger` is a defensive signature in its own right (fires on close-mark conditions) but doesn't yet cancel attacking-signature firings.
- `MatchEvent` proper emission — T1-4. T1-2b-iv uses a local `MemoryEvent` stub for `SignatureFirstFired`; T1-4 reconciles.
- `MemoryEvent` ledger persistence — T3-1. T1-2b-iv emits into a transient `Vec<MemoryEvent>` ON MatchState; T3-1 wires the real ledger.
- Real commentary line bank — `SignaturePresentationRecipe` strings stay placeholder. T2 fills.
- Real camera framing — same.
- `UnknownSignatureClass` mod-compat dispatcher path — types exist (T1-3) but no exercising in T1-2b-iv. Defer.
- Per-player BT runner integration of signature bias INTO every leaf — only the 5 on-ball/off-ball utility scorers (shoot/pass/dribble/press/cover matching SimBiasSnapshot's 5 fields) get the bias composition; reactive predicates + GK FSM defer to a polish pass.

### Plan (7 chunks; TDD RED-GREEN-REFACTOR per chunk; ZERO test deferrals per iii-c/iii-d lesson)
- [x] Chunk 1 (RED+GREEN): MatchState canonical-state extensions. `signature_cooldowns: BTreeMap<(PlayerSlot, SignatureId), Tick>` (cooldown end-tick keyed by player+signature); `signature_firing: [Option<SignatureFiring>; 22]` per-player (`SignatureFiring { id: SignatureId, start_tick: Tick, duration_ticks: u32 }`); `signature_first_fired_seen: BTreeSet<(PlayerSlot, SignatureId)>` for the SignatureFirstFired-once-per-match invariant. Canonical encoder VERSION 4 → 5; wire-format diagram updated. Tests: serde round-trip; default at MatchState::initial is all empty/None.
- [x] Chunk 2 (RED+GREEN): `signature/triggers.rs` — 3 trigger predicate functions. Each takes `(&MatchState, PlayerSlot)` and returns `bool`. **Body shield pressure**: defender within 2m of ball-carrier with marking+strength attributes meeting threshold. **Long range strike**: attacker within shooting range (>25m from goal) with composure+technique meeting threshold + ball in foot-contact range. **First-time diagonal switch**: midfielder with vision+technique meeting threshold + ball arrived in 1-tick range + opponent on one wing. Tests: per trigger, construct a state where it fires + a state where it doesn't fire (boundary conditions).
- [x] Chunk 3 (RED+GREEN): `signature/dispatcher.rs` — `evaluate_signatures(state, player_slot, content_store) -> Option<(SignatureId, SimBiasSnapshot)>`. For each candidate in the player's `signature_candidates`: load the SignatureDefinition; check cooldown via `signature_cooldowns`; evaluate trigger; if eligible, add to candidates vec. If 0 eligible → None. If 1 → return it. If multiple → softmax via `pick_top_n_softmax` with `SeedLayer::SignatureTrigger` site = `(player_id << 16) | local_decision_counter`. Tests: 0/1/2-candidate paths; same-seed-same-input deterministic; cooldown skips.
- [x] Chunk 4 (RED+GREEN): `signature/bias_apply.rs` — `apply_signature_bias(raw_utility, signature_def, consideration) -> Q32` where `consideration` is one of (Shoot/Pass/Dribble/Press/Cover) mapping to a SimBiasSnapshot field. Tests: known bias values produce expected output; no-op snapshot (all 1.0) returns raw_utility unchanged.
- [x] Chunk 5 (RED+GREEN): wire into `dispatch.rs::dispatch_tick`. After per-player `should_decide` fires, evaluate signatures; if a signature fires for the FIRST time, set `signature_first_fired_seen.insert((slot, sig_id))` and emit `MemoryEvent::SignatureFirstFired` (local stub). Set `signature_firing[slot]` + `signature_cooldowns[(slot, sig_id)] = current_tick + cooldown_ticks`. Modify `bt/on_ball.rs` + `bt/off_ball.rs` so utility-scoring composes the signature bias when `signature_firing[slot].is_some()`. Tests: integration — signature fires → next tick's utility reflects the bias multiplier → subsequent ticks within cooldown window don't re-fire.
- [x] Chunk 6 (RED+GREEN): 3 RON signature fixtures + tests they load + their trigger functions are correctly bound to their IDs (via a `BTreeMap<SignatureId, fn(&MatchState, PlayerSlot) -> bool>` binding table in `signature/triggers.rs`). At least one player fixture references all 3 signatures with documented affinities.
- [x] Chunk 7 (RED+GREEN + REBASELINE): 6+ proptest invariants in `signature_dispatcher_proptest.rs` covering acceptance criteria 1-5: (a) trigger determinism (same state + slot → same bool); (b) dispatcher determinism (same seed + state → same picked signature); (c) cooldown enforced (signature fires at tick t; doesn't re-fire until t + cooldown); (d) softmax distribution (over many seeds, all eligible candidates can be picked); (e) bias multiplication composes (utility-with-firing = utility-without-firing × bias-factor within ULP tolerance); (f) SignatureFirstFired emitted once per (player, signature) pair per match. **Rebaseline canonical hash.** Commit-body marker: `canonical hash: REBASELINED (trigger: #1 schema bump — MatchState gained signature_cooldowns + signature_firing + signature_first_fired_seen; encoder VERSION 4→5; old: blake3:1db6020c…59c798; new: blake3:<NEW>; reason: T1-2b-iv signature dispatcher)`.

### Self-review outcome
Initial implementation passed `scripts/fw verify` but self-review triple returned **BLOCK + Revise + Revise**: all 6 shipped AC tests were structurally vacuous (recurrence of the iii-c/iii-d pattern), plus 2 real semantic bugs (`signature_memory_events` lifecycle footgun + `Idle`→`Cover` wildcard bias bug). Consolidated fix pass closed 3 P0s + 6 P1s in-place: 6 AC tests rewritten so each 1:1-asserts its MEMORY criterion via the dispatch path (not direct state mutation); `signature_memory_events` got `#[serde(skip)]` + cleared at top of `dispatch_tick` + new `signature_memory_events_not_in_canonical_encoding` unit test; `Idle`→`Neutral` mapping via new `BiasConsideration::Neutral` variant with explicit-enumeration in `intent_to_bias_consideration` (no wildcard); `apply_signature_bias` `debug_assert!` → `assert!`; `evaluate_signatures` panics on unknown trigger binding (was silently `unwrap_or(false)`); `ron` moved to dev-dependencies; `SignatureFiring` fields `pub(crate)` with `::new()` + `id()`/`start_tick()`/`duration_ticks()` accessors; `is_active` returns false for retroactive ticks; `PlayerState.signature_candidates` `pub(crate)` with accessors. Canonical hash UNCHANGED at `blake3:18f1776c…a5d048` (P0-2 + P0-3 don't drift the smoke seed because: signature_memory_events was never in the encoder, and the Idle→Neutral path requires non-empty sig_definitions which `tick_match` doesn't provide on the smoke run).

...
- [ ] Content-pack validator: cross-doc event-class enum exact-match check — `EventClass.SignatureAwakened` / `SignatureExecuted` / `ScoutReportConfirmed` / `ScoutReportDisagreement` names verified against `design/signatures.md` + `design/scout-disagreement.md` + ADR-0005 / ADR-0007 referenced terminology. Rename in any one surface = validator failure per ADR-0004 cross-doc-stability constraint. **Also extends to `CallbackTag.ConsumingReaders` validation (≥1 reader per tag).** Phase-6 implementation wraps into the Tier-A + Tier-D validator subsets

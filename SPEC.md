...
- [x] ADR (7): Scout archetype schema + ScoutReport schema + gate-fallback (2026-04-24 — `design/adr/adr-0007-scout-archetype-schema.md` authored as **Proposed**. Commits BOTH paths architecturally: Path A 3 MVP archetypes (PhysicalProfiler/TechnicalPurist/RegionalExpert) if Month-4 gate passes; Path B `BasicScoutUncertainty` fallback if gate fails. Identical `ScoutReport` schema across both paths so `signatures.md §Q5` counterplay surface is stable regardless. `ScoutReport.EmitsOn*` references `Memory.Contracts` event-class constants. `Scout.Biases.NarrativeFlag = 0` validator invariant. Five rejected alternatives. 3 asmdefs: Scouting.Contracts / .Runtime / .Prototype (Prototype project is explicit throwaway))

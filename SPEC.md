...
- [x] ADR (5): SignatureSO schema (2026-04-24 — `design/adr/adr-0005-signature-so-schema.md` authored as **Proposed** with user's 6 pre-draft constraints baked in: (1) event names via `FinalWhistle.Memory.Contracts` const references not duplicate strings; (2) explicit SignatureScope enum (player/defensive_line/press_unit/set_piece_context); (3) dependency metadata non-behavioral — gates scheduling + validation only, never runtime; (4) field-level capped stacking with stable SignatureSO.Id-tiebreak ordering, no dictionary iteration; (5) DisplayName/UiDescription separated from internal enum IDs — banned-term lint targets player-facing only; (6) latent affinity lives in IdentityPacket per ADR-0006, explicitly NOT here. Five rejected alternatives)

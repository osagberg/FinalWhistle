...
- [x] ADR (6): IdentityPacket / AI Content Compiler (2026-04-24 — `design/adr/adr-0006-identity-packet-compiler.md` authored as **Proposed**; locks IdentityPacket schema with walled-off InternalGeneSnapshot + 46-label phenotype enum with banned-term-lint on rendered strings + SignatureCandidate affinity (receives ADR-0005 handoff) + category-level scout bias + content-pack-qualified stable IDs with no pack-minor leak + canonical-JSON-artifact compiler discipline + 4-project split: Content.Contracts / .Compiler / .Validator / .UnityImport. Five rejected alternatives including runtime LLM and per-field scout bias)

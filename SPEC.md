...
- [x] **ADR (2): Viewer rendering pipeline + URP custom-pass ordering** (2026-04-24 — `design/adr/adr-0002-viewer-rendering-pipeline.md` authored as **Proposed**; Knowledge Risk MEDIUM flagged — Render Graph API verification required at Phase 3 Week 1 spike against pinned Unity 6 LTS URP 17+ before authoring starts Week 2)
- [x] **ADR (3): Production pipeline** (2026-04-24 — `design/adr/adr-0003-production-pipeline.md` authored as **Proposed**; 5-tier model + 5-channel build metadata + runner policy + cost discipline + release-gate rules formalized. Four rejected alternatives: paid pipeline services, all-GitHub-Actions (no Tier C), auto-deploy-to-Steam-on-tag, single-tier mega-workflow. Phase-1 validation criteria already satisfied)

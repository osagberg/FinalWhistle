...
//!
//! ## `raw` input contract: non-negative, NOT capped at 1.0
//!
//! Each `apply_*_bias` asserts only `raw >= 0`. The upper bound is deliberately
//! NOT `<= 1`: the utility functions in `bt/off_ball.rs` / `bt/on_ball.rs`
//! multiply a `[0, 1]` attribute product by personality bonus factors of the
//! form `(1 + 0.1·attr)`, so an elite player legitimately produces `raw > 1.0`
//! (e.g. `utility_hold_formation` reaches 1.1, `utility_run_off_ball` reaches
//! 1.21 at maxed attributes). The biased output feeds the per-candidate softmax,
//! which is well-defined for any non-negative input. Capping `raw` at 1.0 would
//! flatten elite players — the opposite of the Slice-0 elite-skew intent — and
//! the over-strict `raw <= 1` assert previously PANICKED the match in release
//! for high-attribute (procedurally-generated career) players.
//!
...
///
/// Bias inputs are Q32 in `[0, 1]`; `raw` need only be non-negative (it may
/// exceed 1.0 for elite players — see the module-level `raw` input contract).
/// Returns Q32.
pub fn apply_shoot_bias(raw: Q32, attrs: &PlayerAttributes, pressure: DefenderPressure) -> Q32 {
    assert!(raw >= Q32::ZERO, "raw must be >= 0");
    assert!(

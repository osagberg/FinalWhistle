...
- **Youth-player career-path risk.** Players who don't progress get phenotype labels within the locked catalog per `design/player-generation.md` (`"Quietly Gone"` / `"Did Not Kick On"` / `"Returned to Semi-Pro"`) — never stigmatizing framings like "failure" / "washout" / "wasted talent". Compiler generates neutral-or-compassionate narrative language around development shortfalls.

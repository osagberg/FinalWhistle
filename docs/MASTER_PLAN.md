...
- **Phase:** T1 (First Match) — **7 of 11 numbered rows DONE.** Closed since the 2026-05-13 snapshot: T1-1 / T1-2b sub-phase / T1-3 / T1-4a + T1-4b / T1-3.5 / T1-5 / T1-6 / T1-7 / T1-10 / T1-11 / T1-12 / T1-13 / **T1-3.6 (audit-response, just closed)**. **Codex 2026-05-16 audit-triage P1 sweep COMPLETE (5 of 5 P1s closed)**: ball-actions (T1-3.5+T1-3.6), signatures unreachable (T1-11), IPC split-brain + match_frames unbounded (T1-5), content validation (T1-12), LUT bake (T1-10), frontend test gate + cargo audit/deny (T1-13). **Codex post-T1-7 adversarial multi-agent audit** found that T1-3.5's ball-action code was unreachable in the normal path (BT carrier routing gap); T1-3.6 closes that loop + adds frontend runtime shape guards + canonical-unique-attribute test. **The match-engine vertical is now ACTUALLY playable end-to-end** (was claimed-complete at T1-6/T1-7 but Codex's audit reproduced ball-not-moving in 600-tick runs; T1-3.6 rebaselines canonical hash to the new "football actually happens" value).
- **Remaining T1 rows:** T1-8 (replay corpus fixture #1) + T1-9 (behavioral assertions). After both ship the T1 phase ready for `/done` + Codex Tier-3 phase-boundary review per ADR-0015.
- **Build health:** `scripts/fw verify` green; determinism-audit clean; cargo audit + cargo deny green; `pnpm test` 56 tests across 4 files (Match + FrameSource + TacticalBoard + runtime-validators). Canonical hash pinned at `blake3:ddccaf88c94f328274d484ed1e14ced8638d1ccf63bb922ad64a4f28664000b3` post-T1-3.6 rebaseline (was `782fcde6…8c0f`; the rebaseline IS the proof T1-3.5's "vertical complete" was previously not true).
- **Codex audits on disk:** `docs/audits/codex-full-audit-2026-05-13.md` + `docs/audits/codex-pre-t1-2b-prompt.md` + (in-conversation) the 2026-05-16 audit-triage findings + the post-T1-7 adversarial multi-agent audit.
- **Carry-forward set:** mostly consumed; remainder per `REFERENCES.md` "Carry-forward debts" lands during T2.

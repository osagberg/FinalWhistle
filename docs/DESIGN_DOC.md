...
Five differentiator pillars define what makes Final Whistle *distinctive* — and they all sit on top of **Pillar 0 — Believable Football**, the foundation that makes the game a game. A differentiator pillar that doesn't serve its promise is a candidate for cutting; a differentiator that sits on an unbelievable match is emotionally inert regardless of how well it is built.

### Pillar 0 — Believable Football (the foundation)
**Intent:** Before any differentiator can land, the match must read as recognisable football — not just at the level of individual dots moving and a plausible scoreline, but as TEAM SHAPE: a defensive block with a held line and compactness, coordinated pressing lines, a midfield that builds and keeps possession, offside, and the legible phases of play (build-up, transition, sustained pressure, defending a lead). The five differentiator pillars below are **parasitic on this one**: a procedurally unique world, a career that remembers, a hard-won breakthrough, a scout's disagreement, and a signature move are all emotionally worthless if the football they sit inside reads as nonsense. **Expression in gameplay:** a viewer watching the 2D board or reading the commentary recognises the patterns of real football — teams hold and break shape, the press is a line not a swarm, goals come from believable phases, and a better-organised side beats a worse one more often than chance. **Why it is Pillar 0, not Pillar 6:** the determinism contract is defended by clippy lints + a blocking CI gate; believability must be defended with co-equal force. A mean-38.85-goals/match bimodal engine passing four phase gates (T0→T4) undetected is the proof that an assumption is not a guard. Believability is therefore a **standing gate**, not a one-time milestone: a full-match, differentiated-roster goal-DISTRIBUTION check (the `drama_sweep` realism guards M1/M2/M8 + an M1 shape/unimodality guard, run over fixed seeds on non-mirror rosters) is wired into CI alongside the determinism gate, so calibration cannot silently regress and team shape, once built, cannot silently rot. Tuning bands live in `docs/design/drama-model.md`; the gate contract is in `docs/specs/determinism-gate.md` (extended) and MASTER_PLAN Tier F.

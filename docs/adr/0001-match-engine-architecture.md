...
4. **Maintainability under Claude + solo human.** The architecture must be debuggable by reading it, instrumentable without ceremony, and grow in well-defined seams rather than ad-hoc cascades. The prior-art Rust football sims surveyed during T1 design research (research wave 2) both fail this — one of the surveyed projects ships a `forwarders/states/running/mod.rs` that is 2,141 lines of priority cascade; the other surveyed prior-art project punts on per-player decisions entirely. We take neither path.
...
- **60 Hz integration tick.** This is the canonical-state advancement rate — ball physics, player position integration, reactive interrupts, steering. We pick 60 Hz over the 100 Hz rate seen in one of the surveyed prior-art Rust sims because the precision is unnecessary for our text-first surface, and we pick 60 Hz over FM's 4 Hz because we need finer granularity for ball-physics events (deflections, ricochets, near-touches) and signature-move trigger windows — these are the readable atoms of pillar 5, and 250 ms of dead time between checks is too coarse for them.
...
- **Tactic state machine: event-driven + 2 Hz heartbeat.** Transitions fire on possession change, shot, set-piece award, scoreline-tipped events. The 2 Hz heartbeat catches drift the events miss (e.g. gradual territorial shift). This is slower than the 10-tick refresh (10 Hz at 100 Hz tick rate) seen in one of the surveyed prior-art Rust sims because tactic state is meant to be *coarse* — it parameterises everything below, so churn here ripples expensively. Event-driven transitions with a slow heartbeat keep the team intent stable enough to read in commentary.
...
- **Per-role FSM with no utility / no bias vector / no influence maps (one of the surveyed prior-art paths).** Rejected: this is the path one surveyed prior-art project took, and the cost is a 2,141-line single-state file because everything that would otherwise live in a utility selector or an influence-map lookup ends up as a priority cascade in the FSM body. The per-role FSM remains viable for layer 2 (ADR-0006 will decide) but not at the cost of folding layers 3-5 into it.
- **Minute-event stochastic abstraction (the other surveyed prior-art path).** Rejected: incompatible with pillar 5 (24 readable signature moves at 60 Hz cadence), pillar 1 (the procedural fantasy world needs to be *seen* moving, not narrated as aggregate samples), and the determinism floor (their `f64` zone arithmetic and `HashMap` accumulators violate our sim rules outright). That surveyed project is a useful reference for the `MatchCommand` IPC shape and the zone-classification overlay, but not for the decision substrate.
...
- Prior-art research (archived privately): cross-cutting sports-sim architecture synthesis; FM 4 Hz cadence + mid-slice interrupt pattern + hierarchical tactic composition; personality-as-bias-vector + memory-as-thoughts patterns from DF / CK3 / RimWorld / The Sims; layered BT + utility + influence-maps + steering stack with GOAP/HTN rejection rationale; closed-form xG / xT math driving utility scores.
- Prior-art Rust football sims surveyed during T1 design research (research wave 2, archived privately): one minute-event-based prior-art project — adopted its `MatchCommand` shape, rejected its per-player AI substrate; another prior-art project running a 10 ms / 100 Hz FSM approach — adopted its full-tick / light-tick alternation idea, rejected its priority-cascade scale and float / HashMap canonical state.

...
- **ADR-0008 Browser-dev mode for the 2D tactical board** (Proposed) lands the Claude-Preview-MCP-driven visual verification path for T1-2a. Adds a `dump_frames` binary + a URL-param-selected `FrameSource` trait in the frontend. Unlocks AI-paced "is this football?" verification (Claude takes screenshots, no human-in-the-loop required). T1-2a done-criteria updated to require the browser-dev mode end-to-end.
- **Superpowers TDD skill mandate (DECISIONS.md 2026-05-13):** the `superpowers` plugin's RED-GREEN-REFACTOR cycle is mandatory from T1-2b onward for all behavior code. T1-1 (schema/RON) is exempt. Documented in `.claude/skills/next/SKILL.md` Step 4.
- **Claude Preview MCP setup is on the user's side** — when the dev-server prompt fires in the Claude Code app, click through to install the MCP. Until then the `mcp__Claude_Preview__preview_*` tools are deferred-only; after install they activate and ADR-0008's workflow becomes runnable.

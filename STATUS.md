...
**ADR-0001 + 0002 + 0003 + 0004 + 0005 Accepted. ADR-0006 Proposed** (just drafted; expects a pre-review pass like ADR-0005 got since Phase-0 player-generation resolution did most of the heavy lifting). `/next` picks up **ADR (7): Scout archetype + ScoutReport schema + gate-fallback behavior** — consumes ADR-0006's `InternalGeneSnapshot` via category-level bias filter; conditional-MVP per Month-4 feel-test.

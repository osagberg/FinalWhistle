...
- **Frontend / match visual iteration — the loop is "render to a PNG, the agent reads the PNG."** Two interchangeable drivers; both drive a headless browser against the Vite dev server (`pnpm dev`, port 1420) with no Tauri shell:
  - **Playwright harness — DEFAULT, and the ONLY visual loop available in a CLI session** (Claude Preview is Desktop-app-only). Drives system Chrome via `playwright-core` (`channel: "chrome"`, headless — no browser download). For the **2D tactical board / per-tick match feel**: generate a deterministic fixture from current code — `target/release/dump_frames --seed 0x… --ticks N --content content --compact > frontend/public/dev-fixtures/<name>.json` — then `node frontend/scripts/board-shots.mjs /dev-fixtures/<name>.json <comma-ticks>`. It scrubs to each tick via `window.fwDev.scrubTo`, writes one PNG/tick to `/tmp/fw-board/`, and prints a JSON summary (frameCount, per-tick on-screen status, `consoleErrors`). **Read the PNGs.** For **frontend UI screens**: `pnpm screenshots` (`frontend/scripts/capture-visual-screenshots.mjs`) — add a `{route,file}` entry, then read the PNG from `docs/visual/`. Deterministic, headless, CI-portable; this is the standard `/next` Step 5.1 driver.
  - **Claude Preview MCP — Desktop-only optional live pane.** Inside the Claude desktop app the `mcp__Claude_Preview__*` tools (`preview_start` / `preview_eval` / `preview_screenshot` / `preview_console_logs` / `preview_resize`) are available and give a live human-watchable pane. **Detection:** if those tools aren't present, you're in a CLI session → use the Playwright harness. Notes: routes are history-mode (navigate with a full `window.location.href = 'http://localhost:1420/<path>'`, not `window.location.hash`); the viewport can come up collapsed — `preview_resize` to 1280×800 first.
  - **Both drivers, both surfaces:** preview-verify only **browser-safe routes** — anything that calls Tauri `invoke` at mount (`/match`, `/squad`, …) errors gracefully in a plain browser. Safe routes: `/settings`, `/dev/board?source=fixture:/dev-fixtures/<name>.json` (fresh fixture), `/dev/board-preview` (committed fixture). A clean console (`level: "error"`) means no NEW error vs the pre-existing Squad/Career `invoke`-undefined noise (the `getSettings`/`invoke` fallback + a 404 asset are that expected baseline).
- **Save / migration work:** every schema bump owes four tests (forward-migration + callback-preservation + forward-incompat-failure + round-trip-byte-identical) per `docs/specs/save-migration-fixtures.md`.

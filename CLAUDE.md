...
**Four skills wire this up — agent-facing, not user-facing. The user describes a task in natural language; the appropriate skill auto-applies:**

- **`/duo-debate`** — Tier-1 architectural debate. NO repo changes. Already proven by the `2026-05-09-mcp-migration-debate` dogfood. Use BEFORE Tier-2 if design is not settled.
- **`/duo-implement`** — Tier-2 implementing-agent orchestrator (Claude-side). Reads task-spec → plans → implements chunk-by-chunk → runs `fw verify` → posts `commit-proposal` → waits for reviewer ack → commits. Honors escalation triggers (out-of-scope file / design-doc / SPEC mutation / asset-gen call / `fw verify` red >2 / canonical-hash drift / deadlock / budget 80%). Auto-rollback on canonical-hash drift via `.claude/hooks/canonical-hash-guard.sh`.
- **`/codex-review-loop`** — Tier-2 reviewing-agent continuous-polling mode (Codex-CLI-side). The user activates this on the Codex side once per session; Codex polls `scripts/agent-bus pending-reviews` every 4-5 min, reviews each topic with an unreplied commit-proposal, posts `ack` (approve commit) or `counter` (block + reasons). NO repo changes by Codex during review.
- **`/check-reviews`** — Tier-2 review-pickup with cascade-prevention (Claude-side). Polls for Codex's review-findings on topics Claude implemented; for each, runs `scripts/agent-bus cascade-check --files <csv> --target-topic <topic>` BEFORE applying (exit 0 = safe to apply; exit 4 = defer to user). Prevents cascading bugs when Claude has moved on to a downstream task that depends on the same files.

**Coordination model (steady state):** the user issues a `task-spec` (via Claude, who phrases the structured fields from natural-language description and posts `from=user`). Claude executes `/duo-implement` autonomously. Codex runs `/codex-review-loop` in parallel. Claude periodically invokes `/check-reviews` to pick up Codex's findings without breaking flow. Cascade-prevention gates the fix-application so Claude doesn't auto-mutate files in scope of downstream in-flight tasks. The user reviews diffs at next check-in. **The user never invokes `scripts/agent-bus` directly** — the skills do.

...
- **Frontend iteration — preferred tooling (user preference):** iterate on frontend with the **Claude Preview MCP** (`preview_start` / `preview_eval` / `preview_screenshot` / `preview_console_logs` / `preview_resize`), run from **Claude Code inside the Claude desktop app** — that's where the preview MCP is available, and the live preview + screenshot + console-log loop iterates far faster than blind edits. This is the standard path for the `/next` Step 5.1 preview check AND for any frontend dev loop. Notes from prior use: the app routes are history-mode, so navigate with a full `window.location.href = 'http://localhost:1420/<path>'` (not `window.location.hash`); the preview viewport can come up collapsed — call `preview_resize` to 1280×800 before screenshotting; Tauri-only routes (anything calling `invoke`) error gracefully in the browser, so preview-verify against browser-safe routes (`/settings`, `/dev/board-preview`, any route that doesn't `invoke` at mount). A clean console (`level: "error"`) means no NEW error vs the pre-existing Squad/Career `invoke`-undefined noise.

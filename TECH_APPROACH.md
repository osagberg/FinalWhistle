...
## 11. Editor automation surface (Phase 3+)

From Phase 3 onward, all Unity Editor automation invoked by Claude Code, Codex CLI (when explicitly granted the slot), or any subagent flows through the **official Unity AI Assistant MCP** (`com.unity.ai.assistant 2.7.0-pre.3`, server name `UnityAIAssistant` in `.mcp.json`, stdio relay child of the Editor process). This is the primary surface. CoplayDev `com.coplaydev.unity-mcp` (server name `UnityMCP`, HTTP :8080) is retained as fallback for the capability gaps named in [`docs/tooling/unity-mcp-routing.md`](docs/tooling/unity-mcp-routing.md) and as entitlement-failure recovery. Authority: [ADR-0011](design/adr/adr-0011-unity-ai-assistant-mcp-migration.md) (Accepted 2026-05-09).

The official MCP is gated behind a Unity Pro seat (`MaxDirect = 1` direct connection slot per seat). The single-driver constraint means Claude holds the cap=1 slot by default; Codex stays as a file-system + `scripts/fw verify` reviewer and does not connect to the Editor MCP unless the user explicitly hands the slot over. The cross-model rhythm in `CLAUDE.md §6.3` (Claude drafts → Codex reviews diffs → Claude applies) is preserved because Codex's review role never required Editor access.

The capability uplift over CoplayDev maps directly to the next four phases. **Phase 3 dots-phase polish** gets purpose-built `Unity_SceneView_Capture2DScene` for L2 visual evidence and `Unity_ManageMenuItem` for arbitrary menu-bar invocation. **Phase 4 narrative + content** can use bake-time asset generation (ElevenLabs TTS for scout-prose drafts, GPT Image 1.5 for crest concepts) without violating the §3 "no runtime LLMs" constraint, because everything runs Editor-time and lands as compiled content. **Phase 5/6 3D-pipeline spike** gets Tripo P1 image-to-3D + Tripo Rigging + Retopo + Texturing + Unity Text-to-Motion — a material accelerant for the production-feasibility spike that currently dominates the candidate-stack risk in `design/3d-pipeline.md`. **Phase 7 perf budget enforcement** gets 11 granular profiler tools enabling programmatic GC-alloc-per-frame and top-time-sample queries from `gameplay-programmer` / `engine-programmer` agents, replacing manual profiler captures. Semantic C# edits (`replace_method` / `replace_class` / `anchor_*`) reduce edit drift on determinism-sensitive code in `MatchSim/Sim/**`.

Two costs are explicitly accommodated rather than hand-waved. **Pre-release churn**: `2.7.0-pre.3` is the third pre in two weeks with breaking tool-shape changes between pres; stable `2.7.0` is unscheduled. We pin to `2.7.0-pre.3` in `unity-project/Packages/manifest.json`, do not auto-upgrade, and treat any new pre/stable release as a tracked SPEC task that re-tests the routing table and amends ADR-0011 if tool shapes shifted. **Entitlement fragility**: the cap=1 entitlement is per active Pro seat; loss of seat or post-beta pricing shift = MCP goes dark. CoplayDev stays installed and registered exactly to absorb this failure. Routing table can shift entries back to CoplayDev primary in <30 minutes by editing `docs/tooling/unity-mcp-routing.md` and `.mcp.json`. This is a two-way door per ADR-0011.

The full task-class → tool routing matrix lives at [`docs/tooling/unity-mcp-routing.md`](docs/tooling/unity-mcp-routing.md). That file is the load-bearing operational artifact: every subagent invocation that may touch the Editor is expected to read it first. The mandatory-rotation table in `CLAUDE.md §6.3` does not duplicate routing — it points there. CoplayDev retirement is gated on five conditions documented in the routing table's "deprecation candidates" section: stable `com.unity.ai.assistant` release, writeable UPM package management on official, dedicated prefab-granular tooling on official (or 30-day confirmation of `Unity_ManageAsset` sufficiency), dedicated animation tooling on official (or same confirmation), and ≥90 days entitlement-model stability post-beta. Until all five are green, CoplayDev stays.

Bake-time content discipline is unchanged. The asset-generation tools surfaced by `UnityAIAssistant` (Flux 2, Gemini 3.x, GPT Image 1.5, Tripo P1/Rigging/Retopo/Texturing, ElevenLabs, Lyria 3, Unity Text-to-Motion, etc.) are bake-time-only by definition — they run in the Editor, write to `Assets/Generated/**` (path TBD), and feed the same content-pack ID + schema-version pipeline as hand-authored content. No runtime LLM invocations; no speculative generation outside a tracked SPEC task. The §3 ban survives intact.

## 12. Cross-agent dialog (Claude ↔ Codex)
